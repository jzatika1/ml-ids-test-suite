...
def load_file(filepath):
    try:
        ...
        with open(filepath, encoding="utf-8-sig") as file:
            ...
            has_header = (
                sum(element.strip()[0].isdigit() for element in elements if element.strip())
                < len(elements) / 2
            )
        ...
        if has_header:
            ...
            try:
                ...
                data = pd.read_csv(
                    filepath,
                    usecols=desired_columns_primary,
                    low_memory=False,
                    encoding="ISO-8859-1",
                )
                ...
                try:
                    ...
                    data = pd.read_csv(
                        filepath,
                        usecols=desired_columns_secondary,
                        low_memory=False,
                        encoding="ISO-8859-1",
                    )
                    ...
                    data.loc[
                        (data["Total Fwd Bytes"] == 0) & (data["Total Bwd Bytes"] == 0),
                        "Total Fwd Bytes",
                    ] = data["Fallback Fwd Bytes"]
                    data.loc[
                        (data["Total Fwd Bytes"] == 0) & (data["Total Bwd Bytes"] == 0),
                        "Total Bwd Bytes",
                    ] = data["Fallback Bwd Bytes"]
                    ...
                    data.rename(
                        columns={"received_bytes": "src_bytes", "sent_bytes": "dst_bytes"},
                        inplace=True,
                    )
            ...
            for pos, name in zip(column_positions, desired_columns, strict=True):
        ...
        X["Protocol"] = X["Protocol"].map(
            lambda value: PROTOCOL_MAPPING.get(
                str(value).strip().lower(), PROTOCOL_MAPPING.get(value)
            )
        )
